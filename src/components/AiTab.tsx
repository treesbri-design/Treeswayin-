...
export const AiTab: React.FC<AiTabProps> = ({
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] pb-20 animate-fadeIn">
      {/* Top AI Bar */}
      <div className="bg-gradient-to-r from-[#1E3A8A] via-[#2448B1] to-[#122452] text-white p-4 rounded-[28px] sm:rounded-[32px] shadow-lg flex items-center justify-between mb-3 border border-blue-700/60">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-2xl bg-[#D4AF37] flex items-center justify-center text-[#1E3A8A] font-extrabold shadow-md">
            <Sparkles className="w-4 h-4" />
          </div>
          <div>
            <h3 className="text-xs font-extrabold tracking-tight">FaithPath AI Guide</h3>
            <p className="text-[10px] text-blue-200">Biblically grounded Q&A & Spiritual Counsel</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-1">
        {messages.length === 0 ? (
          <div className="bg-white rounded-[28px] sm:rounded-[32px] p-6 text-center space-y-4 border border-slate-100 my-auto shadow-lg shadow-slate-200/50">
          ...
          messages.map((msg) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`max-w-[85%] rounded-[22px] p-4 shadow-xs space-y-2 ${
                    isUser
                ...
                >
                  <div className="flex items-center justify-between text-[10px] text-slate-400 border-b border-slate-100/50 pb-1 mb-1">
                    ...
                    <div className="pt-2 border-t border-slate-100 flex flex-wrap gap-1.5 items-center">
                      ...
                      {msg.scriptureReferences.map((ref, idx) => (
                        <button
                          ...
                          className="px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-[#1E3A8A] border border-blue-200/80 rounded-xl text-[10px] font-bold flex items-center gap-1 transition-colors"
                        >
        ...
        {isLoading && (
          <div className="flex gap-2.5 items-center">
            <div className="w-8 h-8 rounded-2xl bg-[#1E3A8A] text-[#D4AF37] flex items-center justify-center text-xs shadow-xs">
              ✝
            </div>
      ...
      {/* Input Box */}
      <div className="mt-2 pt-2 border-t border-slate-100 bg-white rounded-2xl p-2 shadow-lg shadow-slate-200/40">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            handleSendQuery();
        ...
        >
          <input
            ...
            className="flex-1 px-3.5 py-2.5 text-xs text-slate-900 bg-slate-50 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1E3A8A]/30"
            disabled={isLoading}
          />
          ...
          <button
            ...
            className="p-2.5 bg-[#1E3A8A] hover:bg-blue-900 text-white rounded-xl shadow-sm disabled:opacity-40 transition-colors"
          >
